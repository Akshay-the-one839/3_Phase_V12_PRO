...
Serial1.println("$*"+low_vr +"*" +low_vy +"*" + low_vb +"*" + high_vr +"*" + high_vy + "*" +high_vb + "*" +over_rc + "*" +over_yc + "*" + over_bc);
...
Serial1.println("~*"+ cyclic_state + "*" + cyclic_1on+"*"+cyclic_1off+"*"+cyclic_2on+"*"+cyclic_2off+"*"+cyclic_3on+"*"+cyclic_3off+"*"+cyclic_4on+"*"+cyclic_4off+"*"+cyclic_5on+"*"+cyclic_5off+"+");
delay(2000);
cy_time_flag =0;
  }
...
Serial1.begin(9600, SERIAL_8N1, 44, 43);  // PIC
